$(document).ready(function() {
  ...
  test('Strings: join', function() {
    equal(_s.join('', 'foo', 'bar'), 'foobar', 'basic join');
    equal(_s.join('', 1, 'foo', 2), '1foo2', 'join numbers and strings');
    equal(_s.join(' ','foo', 'bar'), 'foo bar', 'join with spaces');
    equal(_s.join('1', '2', '2'), '212', 'join number strings');
    equal(_s.join(1, 2, 2), '212', 'join numbers');
    equal(_s.join('','foo', null), 'foo', 'join null with string returns string');
    equal(_s.join(null,'foo', 'bar'), 'foobar', 'join strings with null returns string');
    equal(_s.join(1, 2, 3, 4), '21314');
    equal(_s.join('|', 'foo', 'bar', 'baz'), 'foo|bar|baz');
    equal(_s.join('',2,3,null), '23');
    equal(_s.join(null,2,3), '23');
  });

  test('Strings: reverse', function() {
    equal(_s.reverse('foo'), 'oof' );
    equal(_s.reverse('foobar'), 'raboof' );
    equal(_s.reverse('foo bar'), 'rab oof' );
    equal(_s.reverse('saippuakauppias'), 'saippuakauppias' );
    equal(_s.reverse(123), '321', 'Non string');
    equal(_s.reverse(123.45), '54.321', 'Non string');
    equal(_s.reverse(''), '', 'reversing empty string returns empty string' );
    equal(_s.reverse(null), '', 'reversing null returns empty string' );
    equal(_s.reverse(undefined), '', 'reversing undefined returns empty string' );
  });
  ...
  test('Strings: include', function() {
    ok(_s.include('foobar', 'bar'), 'foobar includes bar');
    ok(!_s.include('foobar', 'buzz'), 'foobar does not includes buzz');
    ok(_s.include(12345, 34), '12345 includes 34');
    ok(!_s.contains(12345, 6), '12345 does not includes 6');
    ok(!_s.include('', 34), 'empty string includes 34');
    ok(!_s.include(null, 34), 'null includes 34');
    ok(_s.include(null, ''), 'null includes empty string');
  });
